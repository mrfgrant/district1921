...
export async function POST(req: NextRequest) {
  ...
  if (!runRes.ok) {
    ...
    return NextResponse.json({ error: `Outscraper error: ${runRes.status} ${text.slice(0, 200)}` }, { status: 500 })
  }
  ...
  const rows = places
    ...
    .map((p, i) => {
      ...
      return {
        ...
        address:     p.full_address ?? p.street ?? null,
        city:        p.city ?? '',
        state:       stateAbbrev,
        zip:         p.postal_code ?? p.zip ?? null,
        phone:       normalizePhone(p.phone),
        website:     p.site ?? p.website ?? null,
        email:       p.email_1 ?? null,
        description: p.description ?? null,
        lat:         p.latitude ?? null,
        lng:         p.longitude ?? null,
        _hours:      p.working_hours ?? null,
      }
    })
